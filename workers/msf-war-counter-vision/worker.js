...
export function buildVisionPrompt(strategy="grouped_wide_crops"){if(strategy!=="grouped_wide_crops")throw new Error("Stratégie Vision invalide.");return[
"Tu analyses une planche de 10 portraits Marvel Strike Force déjà recadrés et agrandis.",
"La planche contient exactement 2 rangées de 5 cases.",
"Première rangée de gauche à droite: left-1,left-2,left-3,left-4,left-5.",
"Deuxième rangée de gauche à droite: right-1,right-2,right-3,right-4,right-5.",
"Chaque case porte son identifiant au-dessus du portrait. Ignore les scores, cadres, badges et croix rouges lors de l'identification du personnage.",
"Réponds avec un seul objet JSON valide, sans markdown ni texte autour.",
"Schéma exact: {\"schemaVersion\":\"2.1.0\",\"slots\":[{\"slot\":\"left-1\",\"barred\":false,\"candidates\":[{\"name\":\"Nom du personnage\",\"confidence\":0.0}]}]}.",
"Fournis exactement 10 slots dans l'ordre indiqué.",
"Pour chaque slot, donne au maximum 3 noms classés du plus probable au moins probable.",
"barred vaut true si une croix rouge recouvre le portrait, false sinon, null seulement si impossible à déterminer.",
"N'utilise une confiance supérieure à 0.8 que si l'identification visuelle est réellement distinctive."
].join("\n");}
export function buildGroqPayload({env,imageDataUrl,strategy="grouped_wide_crops"}){if(!imageDataUrl?.startsWith("data:image/"))throw new Error("Image Vision invalide.");return{model:getVisionModel(env),messages:[{role:"user",content:[{type:"text",text:buildVisionPrompt(strategy)},{type:"image_url",image_url:{url:imageDataUrl}}]}],temperature:0.2,top_p:0.8,max_completion_tokens:1200,reasoning_effort:"none",response_format:{type:"json_object"}};}
export function parseJsonContent(text){if(typeof text!=="string"||!text.trim())throw new Error("Réponse Groq vide.");const trimmed=text.trim().replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"").trim();try{return JSON.parse(trimmed);}catch{}const start=trimmed.indexOf("{");const end=trimmed.lastIndexOf("}");if(start<0||end<=start)throw new Error("JSON Groq invalide.");try{return JSON.parse(trimmed.slice(start,end+1));}catch{throw new Error("JSON Groq invalide.");}}
