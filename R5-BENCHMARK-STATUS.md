...
Original local/uploaded archive name:
...
## Run #26 — descriptor export for exact 80-crop robustness
...
Run the exact AKAZE robustness benchmark on all 80 JPEG crops from the original R5 ZIP using the exported 450-reference descriptor artifact.

Required report:

- aggregate Top1 / Top5 / Top20 over 80 crops;
- exact rank for every slot × variant;
- results by variant (`base`, `tight`, `loose`, shifts, `red-neutral`);
- results by character;
- identify any systematic failures;
- verify whether red-cross defense crops remain robust under translation/crop changes;
- only after that decide whether AKAZE should enter the WebApp pipeline.

Primary acceptance goal: high Top1 stability and at minimum ≥9/10 characters retaining reliable Top20 recall under realistic crop perturbations.
