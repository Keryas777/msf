(() => {
  ...
  function assertValidatedDraft(draft) {
    ...
    if (!Object.prototype.hasOwnProperty.call(ALLIANCE_RULES, draft.alliance)) {
      throw new Error(`Alliance non prise en charge : ${draft.alliance || "inconnue"}.`);
    }
    if (typeof draft.captured_at !== "string" || typeof draft.source !== "string") {
    ...
    for (const [index, player] of draft.players.entries()) {
      ...
      for (const field of PLAYER_NUMERIC_FIELDS) {
        if (!Number.isSafeInteger(player[field]) || player[field] < 0) {
          throw new Error(`Le champ ${field} du joueur ${index + 1} doit être un entier positif ou 0.`);
        }
      }
      if (player.rank < 1) {
  ...
  function buildBaseMetrics(player, totalDamage) {
    ...
    const successfulAttacks = noAttack ? 0 : Math.floor(player.attack_points / 1000);
    const misses = noAttack ? 0 : player.attacks - successfulAttacks;
    let damageAttacks = noAttack ? 0.1 : Math.ceil(player.attack_points / 1000);

    if (damageAttacks > player.attacks) damageAttacks = player.attacks;
    if (damageAttacks <= 0) damageAttacks = 0.1;

    const rawAverageDamage = player.damage / damageAttacks;
    const damageShare = totalDamage > 0 ? player.damage / totalDamage : 0;
  ...
  function calculateReport(validatedDraft) {
    ...
    const totalDamage = input.players.reduce((sum, player) => sum + player.damage, 0);
    const metrics = input.players.map((player) => buildBaseMetrics(player, totalDamage));
    const averageReference = averageTopThree(metrics.map((entry) => entry.rawAverageDamage));
    const shareReference = averageTopThree(metrics.map((entry) => entry.damageShare));

    const players = metrics.map((entry) => {
      const { player } = entry;
      const scoreActivity = entry.noAttack ? 0 : getActivityScore(player.attacks);
      const scoreEfficiency = entry.noAttack ? 0 : getEfficiencyScore(entry.misses);
      const impactAverage = entry.noAttack || averageReference === 0
        ? 0
        : Math.min(24.5, 24.5 * (entry.rawAverageDamage / averageReference));
      const impactShare = entry.noAttack || shareReference === 0
        ? 0
        : Math.min(10.5, 10.5 * (entry.damageShare / shareReference));
      const scoreImpact = entry.noAttack
        ? 0
        : Math.min(35, impactAverage + impactShare);
      const scoreDefense = getDefenseWinsScore(player.defense_wins)
        + getDeviationsScore(player.defense_bonus);
      const scoreTotalRaw = scoreActivity + scoreEfficiency + scoreImpact + scoreDefense;
      ...
      return {
        ...
        min_deviations_ok: player.defense_bonus >= rules.minimum_deviations
      };
    });
