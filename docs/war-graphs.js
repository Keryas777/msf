Oui, je te le refais proprement. J’ai corrigé le point fragile : les zones colorées ne passent plus par du rgba() dans le SVG, mais par du fill="#hex" + fill-opacity, beaucoup plus fiable.
J’ai aussi corrigé le nettoyage du nom joueur dans le select avec un vrai regex pour retirer (9) au lieu du vieux morceau bizarre.
...
(() => {
  ...
  /*
    Important :
    Dans du SVG inline, fill="rgba(...)" peut être mal interprété selon navigateurs/contexte.
    On utilise donc fill="#hex" + fill-opacity="x" directement dans le rect.
  */
