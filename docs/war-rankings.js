...
(() => {
  ...
  const RANKINGS = {
    ...
    miss_rate: {
      ...
      sub: (p) =>
        `${p.total_misses} raté${p.total_misses > 1 ? "s" : ""} / ${
          p.total_attacks
        } attaques`,
    },
    ...
    avg_defense: {
      ...
      sub: (p) =>
        `${p.defense_wins} victoire${p.defense_wins > 1 ? "s" : ""} déf. • ${
          p.deviations
        } déviation${p.deviations > 1 ? "s" : ""}`,
    },
    ...
    avg_activity: {
      ...
      sub: (p) =>
        `${fmt((p.total_attacks || 0) / Math.max(1, p.wars_played || 1), 1)} attaques / guerre`,
    },
  };
  ...
  function normAlliance(a) {
    ...
    if (n.includes("kronos") || n.includes("cronos") || n.includes("chronos")) {
      return "kronos";
    }
  ...
  function apply() {
    ...
    const rows = allPlayers.filter((p) => {
      ...
      if (
        (currentRanking === "success_rate" || currentRanking === "miss_rate") &&
  ...
  function setRanking(key) {
    ...
    tabs.forEach((tab) => {
      tab.classList.toggle("is-active", tab.dataset.ranking === key);
    });
  ...
  async function init() {
    try {
      ...
      const res = await fetch("./data/war-stats.json?v=" + Date.now(), {
        cache: "no-store",
      });
      ...
      allPlayers = data
        .map((p) => ({
          ...p,
          alliance: normAlliance(p.alliance),
      ...
      $players.innerHTML = `
        <div class="emptyState">
          ❌ Impossible de charger <code>data/war-stats.json</code><br>
